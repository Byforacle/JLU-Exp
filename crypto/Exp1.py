...
def feistel_encrypt(input_bytes: bytes, key: bytes, F):
    output = bytes()
    left_half = input_bytes[BLOCK_SIZE // 2 :]
    right_half = input_bytes[: BLOCK_SIZE // 2]

    left_half_output = right_half
    function_output = F(left_half_output, key)
    right_half_output = [
        pair1 ^ pair2 for pair1, pair2 in zip(left_half, function_output)
    ]
...
def feistel_decrypt(input_bytes: bytes, key: bytes, F):
    output = bytes()
    left_half = input_bytes[: BLOCK_SIZE // 2]
    right_half = input_bytes[BLOCK_SIZE // 2 :]
...
def slice_arr(arr: bytes, size: int) -> list[bytes]:
    s = []
    for i in range(0, int(len(arr)) + 1, size): # '+1': it provides padding 0
        c = arr[i : i + size]
        if len(c) != 0:
            s.append(c)
    return s
...
def encrypt(message: bytes, key: bytes, tap_positions):
    ...
    for enc_ in blocks:
        if len(enc_) < BLOCK_SIZE:
            ...
            encrypted_blocks += block_encrypt(enc_.zfill(BLOCK_SIZE), lfsr) # 字节流前面补0
        else:
